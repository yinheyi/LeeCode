...
*	Modifed Time: 2019年04月13日 星期六 22时42分27秒
...
//
// 参数说明： 
// pArray_:指向数组首元素的指针; nLength_: 数组的长度
static bool AdjustArray(int* pArray_, int nLength_)
{
	// 参数的合法性分析
	if (pArray_ == nullptr)
		return false;
	...
	{
		// 从前往后找到第一个偶数
		while (pArray_[_nStart] & 0x1 && _nStart < _nEnd)
			++_nStart;

		// 从后向前找到第一个奇数
		while (!(pArray_[_nEnd] & 0x1) && _nEnd > _nStart)
			--_nEnd;
...
int main()
{
	...
	std::cout << AdjustArray(_nTest1, 20) << std::endl;
	for (int i = 0; i < 20; ++i)
		std::cout << _nTest1[i] << std::endl;

	std::cout << "第二个结果：" << std::endl;
	std::cout << AdjustArray(_nTest2, 0) << std::endl;

	std::cout << "第三个结果：" << std::endl;
	std::cout << AdjustArray(_nTest3, 1) << std::endl;
	for (int i = 0; i < 1; ++i)
		std::cout << _nTest3[i] << std::endl;

	std::cout << "第四个结果：" << std::endl;
	std::cout << AdjustArray(_nTest4, 2) << std::endl;
	for (int i = 0; i < 2; ++i)
